...
- ( NSString *) getFace_idOnPhoto :( UIImage *) Image {
    FaceppResult * result = [[ FaceppAPI detection ] detectWithURL : nil orImageData : UIImageJPEGRepresentation (Image, 1 ) mode : FaceppDetectionModeNormal attribute : FaceppDetectionAttributeAll tag : nil async : NO ];
    if ([[[result content ] objectForKey : @ "face" ] count ]) {
        NSLog ( @ "Succeed" );
        return [[[[result content ] objectForKey : @ "face" ] objectAtIndex : 0 ] valueForKeyPath : @ "face_id" ];
    } else {
        NSLog ( @ "error" );
        return nil ;
    }
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    NSArray *personNames = [NSArray arrayWithObjects:@"Tam1", @"Tam2", @"Tam3", @"Tam4", @"Tam5", nil];
